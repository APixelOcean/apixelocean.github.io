export const moonSVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -0.5 28 28" shape-rendering="crispEdges">
...
export const sunSVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -0.5 27 27" shape-rendering="crispEdges">
  <metadata>Made with Pixels to Svg https://codepen.io/shshaw/pen/XbxvNj</metadata>
...
export const palette = storable('palette', 0);

export function switchPalette() {
  palette.update(n => (n + 1)%2);
  palette.subscribe(p => setPalette(p))
}

export function setPalette(i) {
  const paletteLink = document.getElementById('palette-ss');
  paletteLink.href = `/style/palettes/palette${i + 1}.css`
  const button = document.getElementById('palette-button')
  button.innerHTML = i === 0 ? moonSVG: sunSVG;
}