  ...
  onMount(async () => {
    ...
    const sketch = (p) => {
      ...
      class Circle {
        ...
        move() {
          let dx = this.x - p.mouseX; // Move away from cursor
          let dy = this.y - p.mouseY;
          let distance = p.sqrt(dx * dx + dy * dy);
          
          if (distance < 100) { // Only move away if the cursor is close
          ...
          // Prevent circles from moving their center outside the canvas
